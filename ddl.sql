...
CREATE TABLE `lms-database`.`user_engagement_mis` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `candidate_id` INT NOT NULL,
  `date_time` TIMESTAMP,
  `cpu_count` INT,
  ...
  `cpu_percent` FLOAT,
  `usage_cpu_count` INT,
  `number_of_software_interrupts_since_boot` INT,
  `number_of_system_calls_since_boot` INT,
  `number_of_interrupts_since_boot` INT,
  `cpu_avg_load_over_1_min` FLOAT,
  `cpu_avg_load_over_5_min` FLOAT,
  `cpu_avg_load_over_15_min` FLOAT,
  ...
  `time_spent_doing_actual_I/Os` BIGINT,
  `number_of_bytes_sent` BIGINT,
  `number_of_bytes_received` BIGINT,
  `number_of_packets_sent` BIGINT,
  `number_of_packets_received` BIGINT,
  `total_number_of_errors_while_receiving` INT,
  `total_number_of_errors_while_sending` INT,
  `total_number_of_incoming_packets_which_were_dropped` INT,
  `total_number_of_outgoing_packets_which_were_dropped` INT,
  `boot_time` TIME,
  `keyboard` FLOAT,
  `mouse` INT,
  ...
  `files_changed` INT,
  PRIMARY KEY (`id`),
  INDEX `fk_candidate_id_user_engagement_mis_idx` (`candidate_id` ASC) VISIBLE,
  CONSTRAINT `fk_candidate_id_user_engagement_mis`
    FOREIGN KEY (`candidate_id`)
    REFERENCES `lms-database`.`fellowship_candidate` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
